...
public class Crawler {
	...
	/**
	 * Mark to save paste and email in mongo database
	 */
	private static boolean confSaveMongo;
	/**
	 * Mark to save paste in XML files
	 */
	private static boolean confSaveXML;
	...
	public static void main(String[] args) {
		if(args.length != 0 && args.length != 2 && args.length != 3){
			System.err.println("Usage: java -jar crawlerPasteBin [-x] [-m [userMongo passwordMongo]]");
			System.exit(0);
		}
		confSaveXML = false;
		confSaveMongo = false;
		if("-x".equals(args[0])){
			...
			if(args.length == 2){
				...
				if(args.length == 3){
					new Crawler(args[1], args[2]).start();
				}
				if("-x".equals(args[3])){
					confSaveXML = true;
				}
	 ...
	 * @param content
	 */
	private void savePaste(String title, String author, String date, String content, String path){
		if(confSaveMongo){
			try {
				...
				out.write("<xml><title>"+title+"</title><id>"+path+"<author>"+author+"</author><date>"+date+"</date><content>"+content+"</content></xml>");
				out.flush();
				out.close();
		...
		if(confSaveXML){
			db.insertPaste(title, author, date, content, path);
		}
	}
